...
# S3D API helpers
# --------------------------
def fetch_s3d_metrics_api(entries: List[Dict[str, Any]], api_url: str, max_retries: int = 3, backoff: float = 1.5) -> Dict[str, Dict[str, float]]:
    metrics: Dict[str, Dict[str, float]] = {}
    # Deduplicate dependencies for query
    dep_map: Dict[str, str] = {}  # key -> version
    for e in entries:
        key = f"{e['package']}@{e['version'] or ''}"
        dep_map[key] = e['version'] or ''
    payload = {
        "dependency_names": [k.split('@')[0] for k in dep_map.keys()],
        "dependency_versions": [v for v in dep_map.values()]
    }
    attempt = 0
    while attempt < max_retries:
        try:
            r = requests.get(api_url, params=payload, timeout=15)
            r.raise_for_status()
            data = r.json()
            for entry in data:
...
def compute_prioritization(cvss: Optional[float], epss: Optional[float], s3d: Optional[float],
                           cvss_weight: float = 0.75, epss_weight: float = 0.25, s3d_weight: float = 0.0) -> Optional[float]:
    # normalize
    total = cvss_weight + epss_weight + s3d_weight
    if total == 0:
        return None
    w_cvss, w_epss, w_s3d = cvss_weight / total, epss_weight / total, s3d_weight / total
    return round((cvss or 0.0) * w_cvss + (epss or 0.0) * w_epss + (s3d or 0.0) * w_s3d, 4)
...
def save_csv(results: List[Dict[str, Any]], path: str) -> None:
    fieldnames = [
        ...
        "cvss", "epss", "epss_percentile", "s3d", "s3d_percentile", "score", "score_alt", "fixed"
    ]
...
# --------------------------
def main():
    parser = argparse.ArgumentParser(description="Scan SPDX SBOM with OSV + EPSS + S3D, export JSON/CSV with prioritization")
    ...
    parser.add_argument("--cvss-weight", type=float, default=0.75, help="Weight for CVSS in prioritization")
    parser.add_argument("--epss-weight", type=float, default=0.25, help="Weight for EPSS in prioritization")
    parser.add_argument("--s3d-weight", type=float, default=0.0, help="Weight for S3D in prioritization")
    parser.add_argument("--s3d-api", default="https://s3d-models-api-973562431566.us-east5.run.app/s3d_model_2/metrics",
                        help="S3D model API URL for metrics")
    ...
    # S3D metrics
    s3d_metrics = fetch_s3d_metrics_api(results, args.s3d_api)
    for r in results:
        key = f"{r['package']}@{r['version'] or ''}"
        sm = s3d_metrics.get(key, {"s3d": 0.0, "s3d_percentile": 0.0})
        r["s3d"] = sm.get("s3d", 0.0)
        r["s3d_percentile"] = sm.get("s3d_percentile", 0.0)
        # Compute final prioritization score
        r["score"] = compute_prioritization(r.get("cvss"), r.get("epss"), r.get("s3d"),
    ...
    if args.top and results:
        ...
        for i, r in enumerate(results[:args.top], start=1):
            print(f"{i}. {r['package']}@{r['version']} | ID: {r['id']} | "
                  ...
                  f"S3D: {r.get('s3d')} | Score: {r.get('score')} | Summary: {r.get('summary')[:80]}{'...' if len(r.get('summary',''))>80 else ''}")
